...
resource "google_cloud_run_service" "pinkzebra_backend" {
  ...
  template {
    spec {
      ...
      containers {
        image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/pinkzebra/backend:1.0.4"
        env {
          name = "DB_USER"
          value_from {
            secret_key_ref {
...
resource "google_cloud_run_service" "pinkzebra_frontend" {
  ...
  template {
    spec {
      service_account_name = "terraform-pinkzebra@ceri-m1-ecommerce-2022.iam.gserviceaccount.com"
      containers {
        image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/pinkzebra/frontend:0.4.0"
      }
    }
  }
}
